...
패키지 관리는 `uv`를 기준으로 합니다. 의존성의 기준 파일은 `pyproject.toml`과 `uv.lock`입니다.

MacBook에 `uv`가 없다면 Homebrew로 설치합니다.

```bash
brew install uv
```

Ubuntu server에 `uv`가 없다면 설치 스크립트를 사용합니다.
...
MacBook은 로봇과 카메라 연결용입니다. Ubuntu server는 학습용입니다. 두 머신 모두 같은 lockfile로 Python 환경을 만듭니다.
...
```bash
uv run python collect_data.py --mode act --task-name demo --num-episodes 3 --skip-robot
```
...
```bash
uv run python train.py --mode act --data-dir ./data/demo --device cpu --epochs 2 --batch-size 2
```

GPU가 있다면 다음처럼 실행할 수 있습니다.

```bash
uv run python train.py --mode act --data-dir ./data/demo --device cuda
```

ACT는 기본적으로 `chunk_size=100`, `n_action_steps=100`, CVAE/KL 학습을 사용합니다. 작은 GPU나 빠른 구조 확인이 필요하면 다음처럼 줄일 수 있습니다.

```bash
uv run python train.py --mode act --data-dir ./data/demo --device cpu \
...
```bash
uv run python train.py --mode pi05 --data-dir ./data/demo --device cuda --batch-size 1
```
...
```bash
uv run python inference.py --mode act --checkpoint ./checkpoints/act_last.pt --test-mode --device cpu
```

실제 로봇에서 실행할 때는 다음처럼 사용합니다.

```bash
uv run python inference.py --mode act --checkpoint ./checkpoints/act_last.pt --language "빨간 컵을 집어"
```
...
```bash
uv run python train.py --mode act --data-dir ./data/demo
```
...
```bash
uv run python train.py --mode act --batch-size 1
```
